# Standard Pyhton Library imports
...
class ArticleDetailView(DetailView):
    model = Article
    template_name = 'blog/article/article.detail.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['related_articles'] = Article.objects.filter(
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            search_results = Article.objects.filter(
                reduce(operator.and_, (Q(title__icontains=q) for q in query_list)) | reduce(operator.and_, (Q(
                    slug__icontains=q) for q in query_list)) | reduce(operator.and_, (Q(body__icontains=q) for q in query_list))
            )
...
class TagArticlesListView(ListView):
    ...
    def get_queryset(self):
        ...
        if tag_name:
            tag_articles_list = Article.objects.filter(
                tags_name_in=[tag_name], status=Article.PUBLISHED, deleted=False)
