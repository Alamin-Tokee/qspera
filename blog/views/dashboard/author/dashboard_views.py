    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == self.SAVE_AS_DRAFT:
            ...
            if article_update_form.is_valid():
                ...
                messages.success(request, f"Article drafted successfully.")
                return redirect(request, self.template_name, self.context_object)
...
class AuthorWrittenArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        written_articles = Article.objects.filter(
            author=request.user.id, deleted=False).order_by('-date_created')
        total_articles_written = len(written_articles)

        page = request.GET.page('page', 1)

        paginator = Paginator(written_articles, 0)
...
class AuthorPublishedArticlesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        published_articles = Article.objects.filter(
            author=request.user.id, status=Article.PUBLISHED, deleted=False).order_by('-date_published')
